...
- **Resolución de entidades (capa primaria anti-homónimos)**: `_resolve_entities()` clasifica cada resultado de búsqueda con un LLM (`prospecto`/`empresa`/`irrelevante`) ANTES del análisis, descartando homónimos y ruido. Schema `ENTITY_RESOLUTION_SCHEMA`, prompt `prompts/entity_resolver.md`. Corporate/Perplexity no se clasifican. Fallback a la heurística `_is_relevant_item` si el LLM falla
- **Sistema de verificación**: Score de calidad (0-100) basado en cruce de fuentes. La confianza "verified" cuenta DOMINIOS web distintos, no scrapers (Google+DDG con la misma URL = 1 fuente); Perplexity = +1 fuente independiente
...
- **Principio anti-homónimos (aprendido a golpes el 11-jun-2026, 5 vectores en un día)**: (1) lo que no pasaría el filtro de la UI tampoco debe llegar al LLM — `_is_relevant_item()` filtra ANTES del verifier; (2) las heurísticas de nombre no resuelven identidad (abc ⊂ abc.com pero es ABC Network) — el veredicto de entidad lo da el LLM con contexto (`sitio_web_corresponde`); (3) el perfil LinkedIn validado por empresa siempre enriquece antes que Perplexity, y la persona de Perplexity se descarta si no menciona la empresa
